...
from src.pretraining_methods.nsp_like.SOP import prepare as sop_prep

import random
import math


def mlm_plus_sop(text_corpus, index, tokenizer, vocab, max_len, is_last=False):
    target_len = max_len
    max_tokens = target_len - 3

    sentence_a, sentence_b, is_swapped = sop_prep.get_sentence_pair(index, text_corpus, is_last)
    ...
    masked_tokens, labels, label_positions = mlm_prep.mask_tokens(tokens, tokenizer, vocab, seq_len)
    ...
    sop_label = [int(is_swapped)]

    segment_ids = ([0] * (len(sent_a_tokens) + 2)) + ([1] * (len(sent_b_tokens) + 1))

    return {
        "bert_input": input_ids,
        "bert_label": {"MLM": mlm_labels, "SOP": sop_label},
